...
Be helpful, concise (under 100 words), and conversational. If users ask about products, mention you can search and show results.`;
        ...
        extractCategoryFromMessage: function(message) {
            ...
            // Books
            if (lowerMessage.includes('book') || lowerMessage.includes('textbook') || lowerMessage.includes('guide') || lowerMessage.includes('manual')) return 'books';
            
            // Furniture - more specific matching
            if (lowerMessage.includes('furniture') || 
                lowerMessage.includes('desk') || 
                lowerMessage.includes('chair') || 
                lowerMessage.includes('table') ||
                lowerMessage.includes('stand') ||
                lowerMessage.includes('laptop stand') ||
                lowerMessage.includes('bookshelf') ||
                lowerMessage.includes('filing cabinet')) return 'furniture';
            
            // Electronics
            if (lowerMessage.includes('electronic') || 
                ...
                lowerMessage.includes('hub')) return 'electronics';
            
            // Stationery
            if (lowerMessage.includes('stationery') || 
                lowerMessage.includes('pen') || 
                lowerMessage.includes('pencil') ||
                lowerMessage.includes('notebook') ||
                lowerMessage.includes('paper') ||
                lowerMessage.includes('calculator') ||
                lowerMessage.includes('marker') ||
                lowerMessage.includes('sticky') ||
                lowerMessage.includes('geometry')) return 'stationery';
        ...
        extractSearchTerms: function(message) {
            ...
            const patterns = [
                /(?:show|find|search|looking for|need|want|suggest|recommend)\s+(?:me\s+)?(?:a\s+|an\s+|some\s+)?(.+)/i,
                /(.+)\s+(?:please|pls)/i,
                /(?:i need|i want|looking for|searching for)\s+(?:a\s+|an\s+|some\s+)?(.+)/i,
                /(?:do you have|any)\s+(.+)/i
            ];
            ...
            for (const pattern of patterns) {
                ...
                if (match && match[1]) {
                    ...
                    extracted = extracted.replace(/\b(for|to|in|on|at|with|from)\b.*$/i, '');
            ...
            // If no pattern matches, try to extract key product terms
            const productTerms = ['laptop stand', 'mouse', 'keyboard', 'headphone', 'desk', 'chair', 'book', 'pen', 'notebook', 'calculator'];
